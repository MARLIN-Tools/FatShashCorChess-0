...
void add_smolgen_bias(const Weights& w,
    ...
    linear.fc_rows_no_bias(x, 64, embedding, sg.compress, Activation::NONE, "smolgen.compress", compressed);
    ...
    linear.fc_rows(compressed_flat,
                   1,
                   64 * hidden_channels,
                   sg.dense1_w,
                   sg.dense1_b,
    ...
    const int smolgen_out = linear.infer_out_dim_no_bias(w.smolgen_w, per_head, "global smolgen_w");
    ...
    for (int h = 0; h < heads; ++h) {
        ...
        linear.fc_rows_no_bias(in_head, 1, per_head, w.smolgen_w, Activation::NONE, "global smolgen apply", out_head);
...
WdlOutput forward_attention_value(const Weights& w, const InputPlanes112& input, LinearBackend* backend) {
    validate_attention_value_shapes(w, false);
    LinearBackend local_backend;
    LinearBackend& linear = backend ? *backend : local_backend;
    ...
    std::vector<float> x;
    linear.fc_rows(token_in, 64, 176, w.ip_emb_w, w.ip_emb_b, default_activation(w), "ip_emb", x);
    ...
    for (std::size_t li = 0; li < w.encoders.size(); ++li) {
        ...
        std::vector<float> k;
        std::vector<float> v;
        linear.fc_rows(x, 64, embedding, layer.mha.q_w, layer.mha.q_b, Activation::NONE, "encoder.q", q);
        linear.fc_rows(x, 64, embedding, layer.mha.k_w, layer.mha.k_b, Activation::NONE, "encoder.k", k);
        linear.fc_rows(x, 64, embedding, layer.mha.v_w, layer.mha.v_b, Activation::NONE, "encoder.v", v);

        std::vector<float> scores(static_cast<std::size_t>(heads * 64 * 64), 0.0f);
        add_smolgen_bias(w, layer, x, embedding, heads, linear, scores);
        ...
        linear.fc_rows(attn, 64, embedding, layer.mha.dense_w, layer.mha.dense_b, Activation::NONE, "encoder.proj", proj);
    ...
    linear.fc_rows(x, 64, embedding, w.ip_val_w, w.ip_val_b, default_activation(w), "ip_val", val_tokens);
